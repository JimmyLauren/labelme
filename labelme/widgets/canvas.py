...
class Canvas(QtWidgets.QWidget):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.num_backups = kwargs.pop("num_backups", 10) #字典的常用方法
        ...
        # jimmy add 添加是否显示关键点骨架功能
        self.points_skeletons = kwargs.pop("points_skeleton", 
                                          {
                                              "use_skeleton": False,  # 是否显示关键点骨架
                                              "skeletons": None,  # 关键点骨架的线段，用点的名字表述
                                          })
        
    def setPointSkeleton(self, points_skeleton):
        self.points_skeletons = points_skeleton
    ...
    def paintEvent(self, event):
        ...
        # jimmy add 给点画骨架
        if self.points_skeletons["use_skeleton"]:
            all_points = []
            for skeleton_shape in self.shapes:
                if skeleton_shape.shape_type == "point":
                    all_points.append(skeleton_shape)
            if len(all_points) >= 2 and len(self.points_skeletons["skeletons"]) >= 1:
                for skeleton in self.points_skeletons["skeletons"]:
                    if len(skeleton) >= 2:
                        for sk_id in range(len(skeleton)-1):
                            p1_name = skeleton[sk_id]
                            p2_name = skeleton[sk_id+1]
                        
                            p1 = None
                            p2 = None
                            
                            for poi in all_points:
                                if poi.label == p1_name:
                                    p1 = poi
                                elif poi.label == p2_name:
                                    p2 = poi

                            if p1 and p2:
                                p.setPen(QtGui.QColor(255, 0, 0))
                                p.drawLine(p1.points[0], p2.points[0])
