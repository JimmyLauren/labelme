...
class MainWindow(QtWidgets.QMainWindow):
    ...
    def __init__(
        ...
        createPointMode = action(
            ...
            lambda: self.toggleDrawMode(False, createMode="point"), #无参数lambda表达式形式。有参数lambda表达式形式为lambda a:b。
    ...
    def skeLenHelp(self):
        print("skeLenHelp")
        QtWidgets.QMessageBox.information(self, "关键点骨架显示", 
                                          '''
首先新建一个.xml文件，然后点击菜单栏中的“文件”-“Load XML Config”按钮，选择刚才创建的.xml文件。
xml文件内容描述如下：
useseg对表示是否显示骨架线，true表示显示，false表示不显示。
seg对表示骨架线的点顺序，用,分隔。
...
<config>
    <useseg>true</useseg>
    <seg>left_eye,right_eye,noise</seg>
    <seg>left_hand,right_hand</seg>
    <seg>left_foot,right_foot</seg>
</config>
<!--- 加载配置，根据点顺序为点画线，注意：只在画点的时候有效 --->
<!--- 添加线段，用,分隔。有多条线就添加多个seg对 --->
''')
    ...
    # jimmy add
    def loadXMLConfigFile(self, filename=None):
        """laod xml config file."""
        if not QtCore.QFile.exists(filename):
            self.errorMessage("Error opening file", "No such file: <b>%s</b>" % filename)
            return False
        
        if not filename.lower().endswith(".xml"):
            self.errorMessage("Error opening file", "Not a valid xml file: <b>%s</b>" % filename)
            return False
        
        file = QFile(filename)
        if not file.open(QIODevice.ReadOnly | QIODevice.Text):
            self.errorMessage("Error opening file", "Failed to open file: <b>%s</b>" % filename)
            return False
        
        dom_document = QDomDocument()
        if not dom_document.setContent(file):
            self.errorMessage("Error opening file", "Failed to parse xml file: <b>%s</b>" % filename)
            file.close()
            return False
        file.close()
        
        dom_root = dom_document.documentElement()
        use_seg = dom_root.elementsByTagName("useseg")
        use_seg = use_seg.at(0).toElement().text().lower() == "true"
        
        segments = []
        seg_elements = dom_root.elementsByTagName("seg")
        if seg_elements.count() <= 0:
            self.errorMessage("Error opening file", "No seg element in xml file: <b>%s</b>" % filename)
            return False
        
        for i in range(seg_elements.count()):
            seg_element = seg_elements.at(i).toElement().text()
            segments.append([x.strip() for x in seg_element.strip().split(",")])
            
        points_skeleton = dict()
        points_skeleton["use_skeleton"] = use_seg
        points_skeleton["skeletons"] = segments
        
        print("points_skeleton:", points_skeleton)
        
        self.canvas.setPointSkeleton(points_skeleton) 
    ...
    # jimmy add
    def loadXML(self, _value=False):
        if not self.mayContinue():
            return
        
        path = osp.dirname(str(self.filename)) if self.filename else "."
        filters = self.tr("XML files (*.xml)")
        
        fileDialog = FileDialogPreview(self)
        fileDialog.setFileMode(FileDialogPreview.ExistingFile)
        fileDialog.setNameFilter(filters)
        fileDialog.setWindowTitle(
            self.tr("%s - Choose An XML file") % __appname__,
        )
        fileDialog.setWindowFilePath(path)
        fileDialog.setViewMode(FileDialogPreview.Detail)
        if fileDialog.exec_():
            ...
            if fileName:
                ...
                self.loadXMLConfigFile(fileName)
